  ...
  var taskConfig = {
    ...
    copy: {
      ...
      build_appjs: {
        ...
        options: {
          // Replace ${VAR} in files with the value of 'process.env.VAR'.
          processContent: function (content, src, dest) {
            return content.replace(/\$\{([a-zA-Z_.]+)\}/g, function(match, p1){
              if (!(p1 in process.env)) {
                grunt.log.error("Unknown ENV var '" + p1 + "' found in '" + src + "'");
                return match;
              }
              var value = process.env[p1];
              grunt.log.ok("Replacing ENV var '" + p1 + "' with '" + value + "' in '" + src + "'");
              return value;
            });
          }
        }
